...
export const getClients = async (searchTerm = "", page: number = 1, limit: number = 10): Promise<Client[]> => {
    ...
    if (!res.ok) throw new Error("Не вдалося отримати клієнтів");
    return res.json();
};

export const getClientById = async (id: string): Promise<Client> => {
    const res = await fetch(`${BASE_URL}/${id}`);
    if (!res.ok) throw new Error(`Клієнт з ID ${id} не знайдений`);
...
export const updateClient = async (
    ...
    if (!res.ok) throw new Error("Не вдалося оновити клієнта");
    return res.json();
};
...
export const deleteClient = async (id: string): Promise<{ message: string }> => {
    ...
    if (!res.ok) throw new Error("Не вдалося видалити клієнта");
    return res.json();
};