...
function watchFiles() {
  serverInit() // Ініціалізація сервера
  if (!option) {
    watch(PATH.scssAllFiles, series(compileScss, compileScssMin, sync)) // Перезавантажуємо після SCSS компіляції
  }
  if (option === '--dev') {
    watch(PATH.scssAllFiles, series(compileScssDev, sync)) // Перезавантаження для dev-режиму
  }
  if (option === '--css') {
